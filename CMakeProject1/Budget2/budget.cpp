...
struct BulkInfo {
    ...
    BulkInfo operator + (const BulkInfo& info) const {
        return { income + info.income, spend + info.spend };
    }
};
...
public:
    ...
    BulkLinearUpdater(const BulkMoneySpender& substract)
        : substract_(substract)
    {}

    void CombineWith(const BulkLinearUpdater& other) {
        tax_.factor *= other.tax_.factor;
        add_.delta = add_.delta * other.tax_.factor + other.add_.delta;
        substract_.delta += other.substract_.delta;
    }

    BulkInfo Collapse(BulkInfo origin, IndexSegment segment) const {
        return { origin.income * tax_.factor + add_.delta * segment.length(),
            origin.spend + substract_.delta * segment.length() };
    }
...
class BudgetManager : public SummingSegmentTree<BulkInfo, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<BulkInfo> {
    ...
    BulkInfo Process(const BudgetManager& manager) const override {
...
struct PayTaxRequest : ModifyRequest {
    ...
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
        factor = 1 - (ConvertToInt(input) / 100.0);
    }

    void Process(BudgetManager& manager) const override {
        manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{ factor });
    }
...
vector<double> ProcessRequests(const vector<RequestHolder>& requests) {
    ...
    for (const auto& request_holder : requests) {
        if (request_holder->type == Request::Type::COMPUTE_INCOME) {
            ...
            auto result = request.Process(manager);
            responses.push_back(result.Calculate());
        }
        else {
