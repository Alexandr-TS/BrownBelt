...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, tree_spends, tree_spends_add;

void Init() {
    tree_values.fill(0);
    tree_add.fill(0);
    tree_spends.fill(0);
    tree_spends_add.fill(0);
    tree_factor.fill(1);
}
...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            ...
            tree_spends[w] += tree_spends_add[v] * (r - l) / 2;
            tree_spends_add[w] += tree_spends_add[v];
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            cout << ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to) << "\n";
        }
