...
public:
    ...
    WriteAccess operator[](const K& key) {
        auto bucket_id = get_bucket_id(key);
        return { 
            lock_guard<mutex>(buckets[bucket_id].m),
            buckets[bucket_id].data[key]
        };
    }
    ...
    ReadAccess At(const K& key) const {
        ...
        return { 
            lock_guard<mutex>(buckets[bucket_id].m),
            buckets[bucket_id].data.at(key)
        };
    }

    bool Has(const K& key) const {
        auto bucket_id = get_bucket_id(key);
        lock_guard<mutex> g(buckets[bucket_id].m);
        const auto it = buckets[bucket_id].data.find(key);
        return it != buckets[bucket_id].data.cend();
    }

    MapType BuildOrdinaryMap() const {
        unordered_map<K, V, Hash> result;
        for (const auto& bucket : buckets) {
            lock_guard<mutex> lg(bucket.m);
...
private:
    size_t get_bucket_id(const K& key) const {
        return hasher(key) % buckets.size();
    }

    struct Bucket {
        mutable mutex m;
        unordered_map<K, V, Hash> data;
    };
