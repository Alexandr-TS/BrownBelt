...
struct AddressHasher {
    size_t operator() (const Address& address) const {
        return s_hasher(address.city) + P * (s_hasher(address.street) + P * i_hasher(address.building));
    }

    hash<string> s_hasher;
    hash<int> i_hasher;
    static const size_t P = static_cast<size_t>(1e9 + 7);
};

struct PersonHasher {
    size_t operator() (const Person& person) const {
        return i_hasher(person.height) + P * (s_hasher(person.name) + P * 
            (a_hasher(person.address) + P * d_hasher(person.weight)));
    }
