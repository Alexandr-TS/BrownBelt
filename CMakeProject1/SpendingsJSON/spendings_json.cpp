...
vector<Spending> LoadFromJson(istream& input) {
    auto root = Load(input).GetRoot();
    vector<Spending> result;
    for (const auto& spending : root.AsArray()) {
        auto& map_spending = spending.AsMap();
        result.push_back({ map_spending.at("category").AsString(),
            map_spending.at("amount").AsInt() });
    }
    return result;
}
